...
const SavedSuccessfullyPopUp = '[role="status"]'  //Saved Successfully
...
Cypress.Commands.add('clickOnSubmitButton', () => {
    ...
    cy.wait(5000)
    cy.contains(submitButton).should('exist').should('be.visible').click('center')
})
...
Cypress.Commands.add('clickOnUploadExcelButton', () => {
    cy.get(uploadExcelButton).selectFile('cypress/fixtures/employee_details (7).xlsx', { force: true })
